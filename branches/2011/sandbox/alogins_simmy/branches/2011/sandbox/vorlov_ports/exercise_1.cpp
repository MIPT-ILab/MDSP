/**
 * @file:exercise_1.cpp
 * Simple model of using of ports
...
#define PORT_LATENCY 1 // latency which port provide date
#define PORT_FANOUT 1 // how many fanouts have the ports
#define PORT_BW 1 // bandwidth of ports
...
public:
    ...
    /* "Clock" C object, i.e. execute all 
     * actionsthat it performs for a cycle
     */
    void clock ( int cycle);
};
    ...
    for ( int cycle = 0; cycle < CLOCK_LIMIT; cycle++)
    {
        ...
        if ( _stop.read( &tmp, cycle))
        {
            cout << "-------------------------------\n\n"
                 ...
                 << "Calculation is COMPLETED in cycle " 
                 << cycle << ".\n\n";
...
void C::clock ( int cycle)
{
    ...
    while( true)
	{
        ...
        {
            cout << "\tread the port from B: data = " 
                 << data << "\n";
