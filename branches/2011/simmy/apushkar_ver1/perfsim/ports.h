...
*/
template<class T> WritePort<T>::WritePort( std::string key, 
                                            hostUInt32 bandwidth, 
                                            hostUInt32 fanout):
    Port<T>::Port( key)
{
 ...
 * If port is overloaded by bandwidth (more than _bandwidth token 
 * during one cycle, asserts).
*/
template<class T> void WritePort<T>::write( T what, hostUInt64 cycle)
{
    ...
    {
    // If no init, asserts
        this->critical( "%s WritePort was not initializated", 
                        this->_key.c_str());
        return;
    }
    if ( _lastCycle != cycle)
    ...
    {
        /*
         * If we can add something more on that cycle, 
         * forwarding it to all ReadPorts.
         */
        _writeCounter++;
        for ( ReadListIt it = _destinations->begin(); 
